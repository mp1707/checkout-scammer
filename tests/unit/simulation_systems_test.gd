...
func _test_suspicion_system() -> void:
	...
	_expect_equal_int(50, customer.current_suspicion_percent, "SuspicionSystem advances suspicion after caught roll")
	_expect_equal_int(0, _suspicion_system.get_customer_hand_stage_index(30, customer.customer_type), "SuspicionSystem maps 30 percent to green hand stage")
	_expect_equal_int(1, _suspicion_system.get_customer_hand_stage_index(31, customer.customer_type), "SuspicionSystem maps 31 percent to yellow hand stage")
	_expect_equal_int(1, _suspicion_system.get_customer_hand_stage_index(60, customer.customer_type), "SuspicionSystem maps 60 percent to yellow hand stage")
	_expect_equal_int(2, _suspicion_system.get_customer_hand_stage_index(61, customer.customer_type), "SuspicionSystem maps 61 percent to red hand stage")
	...
	_suspicion_system.setup_customer(chad)
	_expect_equal_int(30, chad.current_suspicion_percent, "SuspicionSystem starts Chad at 30 percent")
	_expect_equal_int(0, _suspicion_system.get_customer_hand_stage_index(chad.current_suspicion_percent, chad.customer_type), "SuspicionSystem maps Chad's 30 percent start to green hand stage")
