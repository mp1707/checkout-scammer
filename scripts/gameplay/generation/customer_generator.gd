...
func generate_customer_for_context(
	...
	var scripted_customer: ScriptedCustomerResource = registry.get_scripted_customer(day, customer_number)
	if scripted_customer != null and assortment_level == scripted_customer.required_assortment_level:
		_append_scripted_products(customer, scripted_customer.products, product_count, run_seed, day, customer_number)
...
func _append_scripted_products(
	...
	scripted_products: Array[ProductVariantResource],
	product_count: int,
	run_seed: int,
	day: int,
	customer_number: int
) -> void:
	var scripted_count: int = mini(scripted_products.size(), product_count)
	for index: int in range(scripted_count):
		var product: ProductVariantResource = scripted_products[index]
		if product == null:
			continue
